...
let matrixInt = try Matrix<Int>(rows: [
  [11, 30, 32, 3, 10, 12, 22, 25, 24, 17, 27, 14, 1, 8, 9],
  [32, 6, 27, 20, 1, 7, 26, 5, 24, 28, 31, 17, 8, 21, 18],
  [11, 25, 26, 29, 32, 12, 18, 5, 2, 1, 17, 31, 7, 10, 21],
  [6, 25, 16, 19, 9, 27, 31, 18, 30, 11, 32, 10, 14, 1, 2],
...
let additionSuite = BenchmarkSuite(name: "Addition matrix") {
  $0.benchmark("Double") {
    _ = try matrixDouble + matrixDouble
  }

  $0.benchmark("Double.InPlace") {
    var matrix = matrixDouble
    try matrix += matrixDouble
    _ = matrix
  }
  ...
  $0.benchmark("Float.InPlace") {
    var matrix = matrixFloat
    try matrix += matrixFloat
    _ = matrix
  }

  $0.benchmark("Int") {
    _ = try matrixInt + matrixInt
  }

  $0.benchmark("Int.InPlace") {
    var matrix = matrixInt
    try matrix += matrixInt
    _ = matrix
  }
}
